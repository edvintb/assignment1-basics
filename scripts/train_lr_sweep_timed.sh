#!/bin/bash
# Learning rate sweep for a given batch size with runtime cap
# Usage: ./scripts/train_lr_sweep_timed.sh <batch_size> <runtime_minutes> [config_file]
# Example: ./scripts/train_lr_sweep_timed.sh 128 30 config/train_slm_ts.yaml
...
if [ -z "$1" ]; then
    ...
    echo "Usage: $0 <batch_size> <runtime_minutes> [config_file]"
    echo "Example: $0 128 30 config/train_slm_ts.yaml"
    exit 1
fi
BATCH_SIZE=$1

# Runtime limit in minutes (required parameter)
if [ -z "$2" ]; then
    echo "Error: Runtime limit (in minutes) is required"
    echo "Usage: $0 <batch_size> <runtime_minutes> [config_file]"
    echo "Example: $0 128 30 config/train_slm_ts.yaml"
    exit 1
fi
...
CONFIG=${3:-config/train_slm_ts.yaml}
...
echo "Runtime limit per experiment: $RUNTIME_MINUTES minutes ($RUNTIME_SECONDS seconds)"
...
for lr in "${LEARNING_RATES[@]}"; do
    ...
    timeout ${RUNTIME_SECONDS}s bash -c "
    ...
    if [ $exit_code -eq 124 ]; then
        echo "Training reached time limit of $RUNTIME_MINUTES minutes"
